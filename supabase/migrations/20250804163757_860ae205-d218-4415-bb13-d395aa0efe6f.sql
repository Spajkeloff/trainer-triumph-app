...
-- Create function to get trainer earnings
...
AS $$
  WITH trainer_data AS (
    SELECT id, payroll_type, session_rate, package_percentage
  ...
  trainer_sessions AS (
    ...
    JOIN trainer_data t ON s.trainer_id = t.id
    WHERE s.status = 'completed'
  ),
  session_earnings AS (
    SELECT 
      CASE 
        WHEN t.payroll_type = 'per_session' THEN ts.session_count * t.session_rate
        ELSE 0
      END as session_earnings
    FROM trainer_sessions ts
    CROSS JOIN trainer_data t
  ),
  package_earnings AS (
    SELECT 
      CASE 
        WHEN t.payroll_type = 'percentage' THEN 
          COALESCE(SUM(p.amount * (t.package_percentage / 100)), 0)
        ELSE 0
      END as package_earnings
    FROM trainer_data t
    LEFT JOIN public.sessions s ON s.trainer_id = t.id
    LEFT JOIN public.payments p ON p.session_id = s.id
    WHERE p.status = 'completed'
  )
  SELECT 
    ts.session_count,
    se.session_earnings as total_session_earnings,
    pe.package_earnings as total_package_earnings,
    (se.session_earnings + pe.package_earnings) as total_earnings
  FROM trainer_sessions ts
  CROSS JOIN session_earnings se
  CROSS JOIN package_earnings pe;
$$;